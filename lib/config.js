...
const CONFIG_PATHS = [
  `${os.homedir()}/.config/dat-daemon/config.json`,
...
function parseConfiguration () {
  ...
  try {
    ...
    const path = require('first-existing-path').sync(CONFIG_PATHS)
    console.error('Reading configuration from %s', path)
    config = require('fs').readFileSync(path).toString().replace(/\s*\/\/.+$/gm, '')
  } catch (e) {
